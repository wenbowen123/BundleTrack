...
inline __device__ float3x3 evalR_dAlpha(float3 angles)
{
...
inline __device__ float3x3 evalR_dBeta(float3 angles)
{
...
inline __device__ float3x3 evalR_dGamma(float3 angles)
{
...
inline __device__ float4x4 evalRtInverse_dGamma(const float3& angles, const float3& translation)
{
...
inline __device__ float4x4 evalRtInverse_dX(const float3& angles, const float3& translation)
{
...
inline __device__ float4x4 evalRtInverse_dY(const float3& angles, const float3& translation)
{
...
inline __device__ matNxM<12, 6> evalRtDeriv(float CosAlpha, float CosBeta, float CosGamma, float SinAlpha, float SinBeta, float SinGamma, float x, float y, float z)
{
	...
	deriv(0, 3) = 0.0f;														deriv(0, 4) = -SinBeta * CosGamma;									deriv(0, 5) = -CosBeta * SinGamma;
	deriv(1, 3) = 0.0f;														deriv(1, 4) = -SinBeta * SinGamma;									deriv(1, 5) = CosBeta * SinGamma;
	deriv(2, 3) = 0.0f;														deriv(2, 4) = -CosBeta;												deriv(2, 5) = 0.0f;
	deriv(3, 3) = CosGamma * CosAlpha * SinBeta + SinAlpha * SinGamma;		deriv(3, 4) = CosGamma * SinAlpha * CosBeta - CosAlpha * SinGamma;	deriv(3, 5) = -SinGamma * SinAlpha * SinBeta - CosAlpha * CosGamma;
	deriv(4, 3) = -SinAlpha * CosGamma + CosAlpha * SinBeta * SinGamma;		deriv(4, 4) = CosAlpha * CosGamma + CosAlpha * SinBeta * SinGamma;	deriv(4, 5) = -CosAlpha * SinGamma + SinAlpha * SinBeta * CosGamma;
	deriv(5, 3) = CosAlpha * CosBeta;										deriv(5, 4) = -SinBeta * SinAlpha;									deriv(5, 5) = 0.0f;
	deriv(6, 3) = -SinAlpha * CosGamma * SinBeta + CosAlpha * SinGamma;		deriv(6, 4) = CosAlpha * CosGamma * CosBeta + SinAlpha * SinGamma;	deriv(6, 5) = -CosAlpha * SinGamma * SinBeta + SinAlpha * CosGamma;
	deriv(7, 3) = -CosGamma * SinAlpha - CosAlpha * SinBeta * SinGamma;		deriv(7, 4) = -CosGamma * SinAlpha + CosAlpha * CosBeta * SinGamma; deriv(7, 5) = SinGamma * SinAlpha + CosAlpha * SinBeta * CosGamma;
	deriv(8, 3) = -SinAlpha * CosBeta;										deriv(8, 4) = -CosAlpha * SinBeta;									deriv(8, 5) = 0.0f;
}
