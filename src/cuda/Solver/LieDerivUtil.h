...
__inline__ __device__ float3 ln_rotation(const float3x3& rotation)
{
	...
	float sin_angle_abs = length(result);
...
__inline__ __device__ matNxM<3, 6> evalLie_derivI(const float4x4& A, const float4x4& D, const float3& p)
{
	...
	for (unsigned int r = 0; r < 3; r++) {
		for (unsigned int c = 0; c < 3; c++) {
			j0(r, c + 9) = -transform(c, r);
			j1(r + 9, c) = A(r, c);
		}
	}
	const float3x3 RA = A.getFloat3x3();
	for (unsigned int k = 0; k < 4; k++) {
		float3x3 m = RA * VectorToSkewSymmetricMatrix(make_float3(D(0, k), D(1, k), D(2, k))) * -1.0f;
		for (unsigned int r = 0; r < 3; r++) {
			for (unsigned int c = 0; c < 3; c++)
				j1(3 * k + r, 3 + c) = m(r, c);
		}
...
{
	float3 dr1 = make_float3(D(0, 0), D(0, 1), D(0, 2));
